...
@section('admin-page-content')
    ...
    <!-- /.content-header -->
    <form id="create-Form" class="create-Form" action="{{ route('admin.deck.update',['deck'=>$deck->id]) }}" method="post" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <!-- Main content -->
                                                    ...
                                                    <div class="col-12">
                                                        <div class="form-group ">
                                                            <label class="field-name col-12 d-flex justify-content-between" for="clean-deck">牌組名稱
                                                                <span><span id="deckCount">{{$deck->deckUnserialize()['count']}}</span> / 60張</span>
                                                                <span id="clean-btn-">
                                                                    <a id="clean-deck" class="float-right text-danger" type="button" ><i class="fa fa-trash-can">清空牌組</i></a>
                                                                </span>

                                                            </label>
                                                            <input type="text" class="form-control" name="title" id="title" placeholder="牌組名稱" value="{{$deck->title}}">
                                                        </div>
                                                        <div class="overflow-auto" style="height: 100vh;">
                                                            <div class="col-12 row" id="DeckWrapper">
                                                                @foreach($deck->deckCardInfo() as $card_id => $card)
                                                                    <div class="col-md-3 col-6 border p-1 DeckCardInfo" id="DeckCardInfo-{{$card_id}}">
                                                                        <small>{{$card['name']}}</small>
                                                                        <img class="w-100 p-1" src="{{$card['image']}}" />
                                                                        <div class="d-flex justify-content-between">
                                                                            <a class="changeCardNumber" href="javascript:void(0)" data-id="{{$card_id}}" data-model="plus">                                                                            <span class="badge badge-warning ">
                                                                                <i class="fa fa-plus"></i>
                                                                            </span>
                                                                            </a>
                                                                            <span class="badge badge-secondary fa-1x" id="CardNum-{{$card_id}}">
                                                                                {{$card['num']}}
                                                                            </span>
                                                                            <a class="changeCardNumber" href="javascript:void(0)" data-id="{{$card_id}}" data-model="minus">                                                                            <span class="badge badge-warning ">
                                                                                <i class="fa fa-minus"></i>
                                                                            </span>
                                                                            </a>
                                                                        </div>
                                                                        <input type="hidden" name="card_id[]" value="{{$card_id}}">
                                                                        <input type="hidden" name="card_num[]" value="{{$card['num']}}" id="cardNumInput-{{$card_id}}">
                                                                    </div>
                                                                @endforeach
                                                            </div>
                                                        </div>
                            ...
                            <div class="card-body">
                                <div class="col-12 row">
                                    ...
                                    <div class="form-group col-12">
                                        ...
                                        <div class="col-12">
                                            <img id="cardUploadImg" class="w-100 p-3" src="{{($deck->image)??"/storage/image/Admin/600x400UploadImageBackground.webp"}}">
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group col-12">
                                        ...
                                        <select class="form-control" name="competition" id="competition">
                                            <option value="standard" {!! $html->selectSelected('standard',$deck->competition) !!}>標準賽</option>
                                            <option value="expanded" {!! $html->selectSelected('expanded',$deck->competition) !!}>開放賽</option>
                                        </select>
                                    </div>
                                    <div class="form-group col-12">
                                        <label class="field-name" for="is_recommend">是否為推薦牌組</label>
                                        <select class="form-control" name="is_recommend" id="is_recommend">
                                            <option value="1" {!! $html->selectSelected(1,$deck->is_recommend) !!}>是</option>
                                            <option value="0" {!! $html->selectSelected(0,$deck->is_recommend) !!}>否</option>
                                        </select>
                                    </div>

                                    <div class="form-group col-12">
                                        <label class="field-name" for="user_id">所屬會員</label>
                                        <select class="form-control" name="user_id" id="user_id">
                                            <option value="{{$deck->user_id}}">{{$deck->user()}}</option>
                                        </select>
                                    </div>
                                    <div class="form-group col-12">
                                        <label class="field-name" for="code">牌組代碼(完成牌組才產生)</label>
                                        <input type="text" class="form-control" name="code" id="code" value="{{$deck->code}}" readonly>
                                    </div>
                                </div>
                            </div>
